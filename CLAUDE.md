...
- **Automático (recomendado):**
  ...
  - O teste abre `louvai.html` num Chromium headless e valida transposição com
    ...
    **modo Apresentar** (barra compacta, Tom no Ajustes, "livro" entre músicas),
    ...
    Falhou = sai com código ≠ 0 e lista o que quebrou. (~170 verificações.)
...
- `PLANO-ui.md` — polimento de UI/ícones em **ondas**: Onda 1 (ganhos rápidos + ícone do Backup)
  **✅ v0.28.0**; Onda 2 (ícones SVG inline via `ICONS`/`icon()`) **✅ v0.29.0**; **Onda 3**
  (incrementos maiores, um por vez) pendente. Insumo: `ANALISE-ui.md` (G1–G12, M1–M8) e
  `ANALISE-icones.md` (conjunto SVG, `archive`).
...
com diff/detalhes) e **polimento de UI Onda 1 + Onda 2** (v0.28.0–v0.29.0: tokens/Tom/toast/vazios
e **ícones SVG inline unificados**). Ver os `PLANO-*.md`.
Ver `ROTEIRO-louvai.md` (seções 4 e 5). **Próximo passo imediato:**
1. **Onda 3 do `PLANO-ui.md`** (incrementos maiores, **um por vez** = uma versão cada): seções do
   ⚙ Ajustes (M2), unificar linguagem de card (M4), reorganizar o `#reposheet` (M5), arrastar-pra-
   fechar sheets (M3), stagger da lista (M7), etc. Acessibilidade contínua (`:focus-visible`, `--muted`).
2. **Validação visual no celular** (dark/light) das Ondas 1–2 — Tom destacado, toast colorido,
   estados vazios, acorde no claro, e o **novo conjunto de ícones SVG** só se confirmam no palco.
