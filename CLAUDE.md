...
- **Automático (recomendado):**
  ...
  - O teste abre `louvai.html` num Chromium headless e valida transposição com
    ...
    **modo Apresentar** (barra compacta, Tom no Ajustes, "livro" entre músicas),
    ...
    Falhou = sai com código ≠ 0 e lista o que quebrou. (~186 verificações.)
...
- **Auto-sync ao abrir (v0.37.0):** `settings.autoPull` (toggle `#auto-pull` no cartão *Baixar*,
  **off por padrão**). Boot chama `maybeAutoPull()` → `pullRepo({silent:true})`. O `silent` se propaga
  por `importJSON(text,{silent})` → `doImport(...,policy,{silent})`: sem skeleton, **pula a folha de
  conflito** (usa `"mine"` — não duplica, remapeia a escala), sem toast de erro offline; avisa só
  quando há novidade. **Idempotente** (roda a cada abertura). `#imp=` em importação tem prioridade.
...
ao abrir** (auto-sync habilitável do repertório+escalas, v0.37.0). Ver os `PLANO-*.md`.
